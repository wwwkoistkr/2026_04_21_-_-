"""
[v2.8.0] 숫자·금액 중심 7줄 귀납법 포맷 (2026-04-24) + Gemini/OpenAI 2단계 엔진.

## v2.8.0 핵심 수정 (2026-04-24, 가독성 대폭 개선)
----------------------------------------------------
v2.7.0 은 4줄(💰📊🔍📈)이었으나 "원인 설명이 부족하고 수혜주가 명시되지 않음" 피드백 반영.
...
### 주요 변경
  1) MIN_ITEM_CHARS 150→300, ITEM_MAX_OUTPUT_TOKENS 1024→1536
  2) _build_item_prompt() — 7줄 프롬프트로 재작성, 수혜주 명시 강제
  3) _is_item_output_valid() — 7개 이모지 중 최소 5개 + 숫자 6개 이상
  4) _fallback_item_markdown() — 동일 7줄 포맷으로 통일

### 기대 효과
  - 1건 분량 150~250자 → 400~600자 (원인+수혜주 명시)
  - 숫자 포함도 평균 4~8개 → 8~14개
  - 수혜주 명시로 투자 의사결정 직접 지원
...
# 개별 뉴스 요약 기준
MIN_ITEM_CHARS = 300            # v2.8.0: 150→300 (7줄 귀납법, 원인·수혜주·시사점 포함)
ITEM_MAX_OUTPUT_TOKENS = 1536   # v2.8.0: 1024→1536 (7줄 확장 대응, 한국어 약 600~900자)
...
def _build_item_prompt(item: Dict[str, Any]) -> str:
    ...
    # v2.8.0: 7줄 귀납법 포맷 (팩트 2 + 원인 2 + 맥락 1 + 수혜주 1 + 시사점 1)
...
아래 뉴스 1건을 **숫자·금액 중심의 7줄 귀납법**으로 섬세하게 분석하세요.
읽는 데 30~40초 이내로 끝나야 하며, 각 줄은 **독립된 한 문장**입니다.

## 절대 준수 사항 (v2.8.0)
1. **출력 구조**: 제목 1줄 + 7개 이모지 라인(💰📊📉📈🔍🎯📬) + 원문 링크 1줄. **오직 이 형식만** 허용.
2. **숫자 밀도**: 7개 이모지 라인에 **숫자(금액·%·건수·연도·배수·종목코드 등)가 총 8개 이상** 포함되어야 함.
3. **귀납법 7단계**: 팩트 2줄 → 원인 2줄 → 맥락 1줄 → 수혜주 1줄 → 시사점 1줄 순서로 **구체 사실에서 투자 결론**으로 전개.
4. **각 줄 길이**: 이모지 1줄당 **한국어 50~90자** (너무 짧으면 실패).
5. **📬 라인**: 반드시 **"시사점"** 단어와 함께 **투자 행동 지침**(매수/매도/관망) + **목표가 또는 상승여력 %** 포함.
6. **🎯 라인**: 수혜주를 **종목명(종목코드) 목표가(+상승여력%)** 형태로 명시. 원문에 종목 정보 없으면 업계 대표주 제시.
7. **언어**: 원문이 영어여도 자연스러운 한국어로 번역. 모든 문장은 마침표/느낌표로 종결.
8. **추정 금지**: 원문에 없는 수치는 쓰지 말 것. 부족하면 "정보 부족" 또는 "원문 확인 필요" 로 명시.
...
원문에 정보가 부족한 라인은 "정보 부족" 이라고 명시하되, 7개 이모지 라인은 **반드시 모두 출력**하세요.
"""
...
def _is_item_output_valid(text: str) -> Tuple[bool, str]:
    """v2.8.0: 7줄 귀납법 포맷 검증.
    검증 기준:
      - 최소 글자수 (MIN_ITEM_CHARS = 300)
      - 7개 이모지 라인(💰 📊 📉 📈 🔍 🎯 📬) 중 최소 5개 이상 존재 (관대한 기준)
      - '시사점' 키워드 존재 (귀납법 결론 강제)
      - '수혜주' 또는 🎯 라인 내용 존재
      - 출처/카테고리 라인 존재
      - 숫자 최소 6개 이상 포함 (수치 중심 포맷 강화)
    """
    if not text:
        return False, "empty"
    ...
    # 필수 이모지 라인 7종 (최소 5개는 존재해야 함 — 원문 정보 부족 시 일부 누락 허용)
    required_emojis = ["💰", "📊", "📉", "📈", "🔍", "🎯", "📬"]
    present = [e for e in required_emojis if e in text]
    if len(present) < 5:
        missing = [e for e in required_emojis if e not in text]
        return False, f"too_few_emoji_lines({len(present)}/7, missing={missing})"
    ...
    # 수치 포함도 검증 — 숫자 최소 6개 이상 (v2.8.0: 3→6 강화)
    digit_count = sum(1 for c in text if c.isdigit())
    if digit_count < 6:
        return False, f"too_few_numbers({digit_count}<6)"
...
def _fallback_item_markdown(item: Dict[str, Any]) -> str:
    """v2.8.0: AI 전면 실패 시 원본 데이터로 7줄 귀납법 포맷 생성.
    AI 수치 분석이 없으므로 각 라인에 장애 안내 또는 원본 발췌를 넣음.
    숫자 6개 이상 확보를 위해 현재 날짜/랭크/시간 정보를 포함.
    """
    orig = item["original"]
    rank = item["rank"]
    category = item.get("category", "기타")
...
📊 원문 발췌 2: {summary_mid}
📉 원인 분석 불가 — AI 요약 엔진({today}) 일시 장애로 구조 분석 생성 실패 (랭크 {rank}/10).
📈 배경 분석 불가 — 원문 기사({source}) 전문을 직접 확인하시기 바랍니다.
🔍 시장 구조 — 자동 분석 엔진 2단계(Gemini→OpenAI) 모두 실패, 원문 링크 참조 필요.
🎯 **수혜주**: 정보 부족 — 원문에서 직접 종목 정보 확인 필요 (AI 분석 100% 실패).
📬 **시사점**: 자동 분석 실패, 원문 기사 전문 확인 후 투자 판단. 랭크 {rank}위 뉴스 (총 10건 중).
"""
