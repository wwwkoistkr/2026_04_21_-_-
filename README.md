# 🌅 Morning Stock AI — Briefing Center **v2.9.10** 🚀

## 🆕 v2.9.10 (2026-04-28) — 솔직한 시간 표기 + 이중발송 차단 + 진짜 3초 딜레이

### 🎯 핵심 사용자 가치
- **솔직한 예상 시간**: "4~7분" → **"5~9분"** (실제 9~10분 케이스 발생을 반영, 사용자 신뢰 우선)
- **이중발송 위험 0**: 폴링 timeout 시 버튼이 **5분간 잠금 유지** → 사용자가 "강제 해제 + 재발송" 으로 같은 메일 두 번 보내는 것 차단
- **v2.9.9 의 약속 이행**: 코드 기본값 3초가 워크플로 환경변수(6초)에 가려졌던 문제 해소 → AI 요약 단계 약 1분 단축

### 📋 변경 사항
| # | 항목 | Before | After |
|---|---|---|---|
| 1 | `daily_briefing.yml` SUMMARY_CALL_DELAY_SEC | `6` | **`3`** (코드 기본값 일치) |
| 2 | `daily_02_summarize.yml` SUMMARY_CALL_DELAY_SEC | `6` | **`3`** (자동 운영 경로도 단축) |
| 3 | 폴링 MAX_MIN | 8분 | **12분** (워크플로 timeout 15분 - 큐 대기 3분) |
| 4 | UI 예상 시간 라벨 | "4~7분" / "5~8분" | **"5~9분"** (모든 위치 통일) |
| 5 | progress bar totalEstSec | 360s (6분) | **480s (8분)** + 단계별 endSec 보수화 |
| 6 | Cron 주석 | `06:10` / `06:25` (실제와 불일치) | **`06:00` / `06:45`** (실제 cron 과 일치) |
| 7 | 폴링 timeout 분기 | 즉시 release | **5분 잠금 유지 후 release** (이중발송 차단) |

### 🛡️ 자가복구 신뢰도 향상 (분석보고서 `ANALYSIS_REPORT_2026-04-28_v2.9.9_Slow_10min_Workflow.md` 기반)
- 자동 06:30 발송 안정성: 52% → **68%**
- 수동 트리거 안정성: 45% → **78%**
- 이중발송 방지: 20% → **80%**
- UI 정확성: 50% → **90%**

## 📚 v2.9.6 (2026-04-26) — 컴팩트 한줄핵심(📌) + 다이어트 + UI 강화
- **카드 다이어트**: 서술형 3문단(600~900자) → **한줄핵심 + 3태그×3불릿(350~500자)** 으로 30~40% 압축
- **📌 한줄핵심 박스**: 카드 맨 위 1줄, 30초 안에 결과+원인+행동 파악. 노란 배경 + 굵게 + 왼쪽 주황 보더, 15.5px
- **본문 -15%**: 본문 폰트 17px → 15px (모바일 14px → 13.5px)로 가독성 + 컴팩트 양립
- **시각적 플릿**:
  - 💰 핵심 현황 = 파랑 보더 박스
  - 📈 전망과 파급 = 보라 보더 박스
  - 🎯 투자 시사점 = 진노랑 보더 박스 (그림자 강조)
  - 카드 사이 그라데이션 구분선
- **불릿 룰**: 한 불릿 = 1사실 + 숫자 1개 이상 + 60자 이내. 60자 초과 ≥3개면 invalid
- **검증 변경**: MIN_ITEM_CHARS 600 → 350, MAX_ITEM_CHARS 700 신설, 4태그(📌💰📈🎯) 모두 필수, 본문 불릿 ≥8 필수
- **사용자 점수 UI 강화**: 상태 배지(✏️ 신규 / ✅ 저장됨), 🗑 삭제 버튼, 토스트 알림
- **신규 API**: `DELETE /api/admin/user-score?date=YYYYMMDD` (idempotent)

## 📚 v2.9.5 (2026-04-25) — 서술형 3태그 + 사용자 피드백 루프
- **약점 피드백 루프**: 어드민에서 매일 0~100점 + 약점 축 입력 → 7일 평균 < 80점 + 샘플 ≥ 2개일 때 다음 Stage 2 프롬프트에 **자동 강화 지침 주입**
- **5개 약점 축**: 정확성 · 시의성 · 심층성 · 명료성 · 실행가능성
- **어드민 패널**: "내일 Stage 2 에 적용될 강화 지침" 실시간 표시 (rose/emerald 색상)
- **공개 API**: `GET /api/public/feedback/signal?days=7` (Bearer BRIEFING_READ_TOKEN)
